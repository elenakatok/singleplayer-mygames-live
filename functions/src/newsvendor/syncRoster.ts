...
// ═══════════════════════════════════════════════════════════════════════════════
// newsvendorSyncRoster (instructor) — pulls the course roster from the classroom and
// pre-creates a participant doc for every enrolled student, so the dashboard shows
// who HASN'T played and newsvendorScoreAndRecord can grade never-launched students −2.
// Mirrors penniesSyncRoster and pricingSyncRoster exactly, with newsvendor's secret
// and newsvendor's collections.
//
...
// `Authorization: Bearer <NEWSVENDOR_CALLBACK_SECRET>` and body { game_instance_id }.
...
export const newsvendorSyncRoster = onCall(
