...
// ═══════════════════════════════════════════════════════════════════════════════
// pricingSyncRoster (instructor) — pulls the course roster from the classroom and
// pre-creates a participant doc for every enrolled student, so the dashboard shows
// who HASN'T played and pricingScoreAndRecord can grade never-launched students −2.
// Mirrors penniesSyncRoster exactly, with pricing's secret and pricing's collections.
//
...
// `Authorization: Bearer <PRICING_CALLBACK_SECRET>` and body { game_instance_id }.
...
export const pricingSyncRoster = onCall(
