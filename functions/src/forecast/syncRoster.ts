...
// ═══════════════════════════════════════════════════════════════════════════════
// forecastSyncRoster (instructor) — pulls the course roster from the classroom and
// pre-creates a participant doc for every enrolled student, so the dashboard shows
// who HASN'T played and forecastScoreAndRecord can grade never-launched students −2.
// Mirrors penniesSyncRoster and pricingSyncRoster exactly, with forecast's secret
// and forecast's collections.
//
...
// `Authorization: Bearer <FORECAST_CALLBACK_SECRET>` and body { game_instance_id }.
...
export const forecastSyncRoster = onCall(
