...
// ═══════════════════════════════════════════════════════════════════════════════
// scorecardSyncRoster (instructor) — pulls the course roster from the classroom and
// pre-creates a participant doc for every enrolled student, so the dashboard shows
// who HASN'T played and scorecardScoreAndRecord can grade never-launched students −2.
// Mirrors penniesSyncRoster and pricingSyncRoster exactly, with scorecard's secret
// and scorecard's collections.
//
// ⚠⚠ AND IT NEVER WRITES `starts_with`. That field is assigned ONCE, on the student's
// FIRST TOUCH, from a join counter (instance.ts). Pre-creating it here would either hand
// every roster row the same arm, or burn counter ordinals on students who never play —
// and either would break the exact 50/50 counterbalancing spec §2.2 depends on. A
// roster row with no `starts_with` is precisely a student who has not started.
//
// SAFE MERGE: writes ONLY identity fields ({participant_id, game_instance_id, name,
// external_id}). It never writes contracts / starts_with / finished_at / kc_answers, so it can
...
// `Authorization: Bearer <SCORECARD_CALLBACK_SECRET>` and body { game_instance_id }.
// CLASSROOM_ROSTER_URL is project-wide (functions/.env.singleplayer-mygames-live) and
...
export const scorecardSyncRoster = onCall(
