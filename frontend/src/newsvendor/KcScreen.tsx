...
import { newsvendorSubmitKcAnswer, type NewsvendorKcQuestionClient } from './api'
import { card } from './ParamsPanel'

// ═══════════════════════════════════════════════════════════════════════════════
// One knowledge-check question (spec §8). Graded — it is this game's ASSESSED
// component (spec §9.1) — and NOT A GATE.
//
// ⚠ THE INSTANCE'S PARAMETERS ARE DELIBERATELY *NOT* ON THIS SCREEN, which is the
// opposite of pricing's KC. Pricing puts the market beside the question because its
// questions are about reading that market. These questions carry their OWN numbers in
// their stems (P = 120, c = 50, …), chosen to differ from the game the student just
// played so they must recompute rather than recall. Showing the game's parameters here
// would invite exactly the substitution the question set is designed to prevent.
//
...
export function KcScreen({
  ...
  question: NewsvendorKcQuestionClient
  index: number
  total: number
  onDone: () => void
}) {
  ...
  // Instructor-added questions may be free text; the authored ten are always mc.
  ...
  const handleSubmit = async () => {
    ...
    setError(null)
    try {
      const res = await newsvendorSubmitKcAnswer(question.field, value)
  ...
  return (
    <div>
      ...
      <h1
        data-testid="nv-kc-prompt"
        style={{ marginTop: 0, marginBottom: '1.25rem', fontSize: '1.2rem', color: colors.text, lineHeight: 1.45 }}
      >
        {question.prompt}
      </h1>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
            {question.options.map(opt => {
              ...
              return (
                <label
                  key={opt.value}
                  data-testid={`nv-kc-option-${opt.value}`}
                  style={{
                ...
                >
                  <input
                    type="radio" name={`nv-kc-${question.field}`} value={opt.value} checked={selected}
                    disabled={submitting || answered}
      ...
      {error && (
        <p data-testid="nv-kc-error" role="alert" style={{ color: colors.errorAction, fontSize: typography.sizeSm, marginBottom: '0.75rem' }}>
          {error}
        </p>
      )}
      ...
      {answered && (
        <section
          data-testid={!verdict.graded ? 'nv-kc-recorded' : verdict.correct ? 'nv-kc-correct' : 'nv-kc-incorrect'}
          style={{
            ...card,
      ...
      <button
        data-testid={answered ? 'nv-kc-continue' : 'nv-kc-submit'}
